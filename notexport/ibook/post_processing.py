...
class IBookNoteProcessor:
    ...
    def _split_word_and_sentences(self, threshold: int):
        sentence_mask = (
            self._data[CONST_IBOOK.NOTE_COL]
            .str.split()
            .apply(lambda x: len(x) >= threshold)
    ...
    def normalize(self):
        target_column = (
            ...
            if IBookNoteProcessor.LEMMATIZED_COLUMN in self._df_word.columns
            else CONST_IBOOK.NOTE_COL
        )
        self._df_word[target_column] = self._df_word[target_column].apply(
